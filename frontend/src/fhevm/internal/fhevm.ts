import { isAddress, Eip1193Provider, JsonRpcProvider, Contract } from "ethers";
import type {
  FhevmInitSDKOptions,
  FhevmInitSDKType,
  FhevmLoadSDKType,
...
async function getChainId(
  ...
  const chainId = await (providerOrUrl as Eip1193Provider).request({ method: "eth_chainId" });
...
export const createFhevmInstance = async (parameters: {
  ...
  const config: FhevmInstanceConfig = {
    ...
    publicParams: pub.publicParams && pub.publicParams[2048] ? {
      2048: pub.publicParams[2048]
    } as any : undefined,
  };
